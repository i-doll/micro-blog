...
pub async fn aggregated_health(
    ...
    for (name, url) in &services {
        ...
        match state.client.get(&health_url).timeout(std::time::Duration::from_secs(3)).send().await {
            Ok(resp) if resp.status().is_success() => {
                results.insert(name.to_string(), json!("healthy"));
            }
            Ok(resp) => {
                all_healthy = false;
                results.insert(name.to_string(), json!({"status": "unhealthy", "code": resp.status().as_u16()}));
            }
            Err(e) => {
                all_healthy = false;
                results.insert(name.to_string(), json!({"status": "unreachable", "error": e.to_string()}));
            }
        }
    }
